...
| 단일 실행 기준 시간 대비 효율 | Adam, lr=0.001, epochs=20, dropout=0.2, hidden=256 -> 128 -> val 98.41%, test 98.27%, 40.0s |
...
- 손실 수렴 및 과적합/과소적합: 최종 선택 모델인 `Adam, lr=0.001, dropout=0.2, hidden=256 -> 128`은 train loss가 epoch 1의 0.3576에서 epoch 20의 0.0212까지 안정적으로 감소했고, validation accuracy는 96.18%에서 98.41%까지 상승했다. test accuracy도 98.27%였기 때문에 과소적합으로 보기는 어렵다. 또한 train loss만 낮아지고 validation accuracy가 크게 무너지는 흐름도 아니어서 뚜렷한 과적합은 보이지 않았다. 다만 dropout이 없는 후보들은 train loss가 더 낮아도 validation accuracy가 선택 모델보다 낮은 경우가 있어, 적절한 regularization이 필요하다는 점을 확인했다.

- 구조, learning rate, dropout 변경 결과: Adam은 `lr=0.0005`와 `lr=0.001`에서 모두 빠르게 98% 이상에 도달했고, SGD는 낮은 learning rate에서는 충분히 학습하지 못했지만 `lr=0.1`에서는 98%대 정확도를 기록했다. Dropout은 `0.3`보다 `0.2`가 더 좋은 균형을 보였고, hidden size는 기존 `512 -> 256`보다 작은 `256 -> 128` 구조가 파라미터를 약 56.1% 줄이면서도 validation 98.41%, test 98.27%를 달성했다. BatchNorm 위치 변경은 성능 차이가 작았기 때문에 기본 구조인 `Affine -> BatchNorm -> ReLU -> Dropout`을 유지했다.

이번 실험에서 좋은 모델은 단순히 가장 큰 모델이나 최고 단일 정확도를 낸 모델이 아니라, 처리할 데이터의 복잡도에 맞게 충분한 정확도를 더 적은 시간과 비용으로 내는 모델이라고 판단했다. MNIST처럼 비교적 단순한 데이터에서는 큰 `512 -> 256` 모델을 고집하지 않아도 `256 -> 128` 모델만으로 목표 정확도를 넘길 수 있었다. 오히려 간소화된 모델은 파라미터 수와 연산량이 줄어 학습 시간이 짧고, 같은 결과를 얻는 데 필요한 계산 비용도 낮기 때문에 결과 대비 시간과 비용 측면에서 더 효율적인 선택이었다.
